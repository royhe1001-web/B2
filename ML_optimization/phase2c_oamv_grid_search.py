...
try:
    ...
    compute_forte = None
    FORTE_DEFAULTS = {'ice_threshold': 0.20, 'ice_max_pos': 6, 'crash_min': 30}
...
try:
    ...
    preload_sector_data = None
    get_sector_score = None
...
class OAMVSimEngine:
    """Simulation engine with 0AMV market regime and FORTE position sizing."""

    def _b2_forte_max_positions(self, date):
        """B2 FORTE持仓数调节: BB冰点多持股 + QVIX熔断收缩, 默认4."""
        try:
            oamv = self.oamv_df.loc[:date]
            ref_date = date
            # BB冰点检测 -> 多持股
            if 'oamv' in oamv.columns and len(oamv) >= 20:
                bb_mid = oamv['oamv'].rolling(20).mean().iloc[-1]
                bb_std = oamv['oamv'].rolling(20).std().iloc[-1]
                if pd.notna(bb_std) and bb_std > 0:
                    bb_upper = bb_mid + 2.0 * bb_std
                    bb_lower = bb_mid - 2.0 * bb_std
                    bb_pct = (oamv['oamv'].iloc[-1] - bb_lower) / (bb_upper - bb_lower)
                    bb_pct = max(0, min(1, bb_pct))
                    if bb_pct < self.forte_params.get('ice_threshold', 0.20):
                        return self.forte_params.get('ice_max_pos', 6)
            # QVIX熔断 -> 收缩
            import pandas as pd
            from pathlib import Path
            qvix_path = Path(__file__).parent.parent / 'output' / 'qvix_500etf.csv'
            if ref_date < pd.Timestamp('2022-09-19'):
                qvix_path = Path(__file__).parent.parent / 'output' / 'qvix_50etf.csv'
            if qvix_path.exists():
                qvix = pd.read_csv(qvix_path, parse_dates=['date'])
                qvix = qvix[qvix['date'] <= ref_date]
                if len(qvix) >= 10:
                    now = float(qvix['close'].iloc[-1])
                    ago10 = float(qvix['close'].iloc[-10])
                    if ago10 > 0 and now > self.forte_params.get('crash_min', 30) and now/ago10 > 1.6:
                        return 1  # 熔断: 只留1只
        except Exception:
            pass
        return 4  # 默认4只
    ...
    def __init__(self, stock_data, b2_params, oamv_df, oamv_params,
                 ...
                 sector_lookup=None, forte_params=None, stop_config=None):
        self.stock_data = stock_data
        self.b2_params = b2_params
        self.oamv_df = oamv_df.set_index('date')
        self.oamv_params = oamv_params
        self.forte_params = forte_params or FORTE_DEFAULTS
        self.stop_config = stop_config or {}
        ...
        self.filter_stats = {'total': 0, 'total_signals': 0, 'filtered_signals': 0,
                             'filtered': 0, 'pct_pass': 0, 'pct_fail': 0,
    ...
    def extract_signals(self, lookback=None):
        ...
        ban_entry = self.oamv_params.get('oamv_defensive_ban_entry', False)
            ...
            try:
                ...
                for sd in signal_dates:
                    ...
                    if self.mktcap_lookup and self.percentiles:
                        lo_pct = self.oamv_params.get('mktcap_lower_pct')
                        hi_pct = self.oamv_params.get('mktcap_upper_pct')
                        if lo_pct is not None and hi_pct is not None:
                            ok = is_in_pct_range(code, sd, self.mktcap_lookup,
                                                self.percentiles, lower_pct=lo_pct, upper_pct=hi_pct)
                        else:
                            ok = is_in_pct_range(code, sd, self.mktcap_lookup,
                    ...
                    # 成交量过滤: 信号前N天内不放量暴跌(恐慌出逃非洗盘)
                    vol_check_days = int(self.b2_params.get('vol_check_days', 5))
                    vol_spike_thresh = self.b2_params.get('vol_spike_thresh', 1.5)
                    if 'vol_ma20' in sdf.columns and vol_check_days > 0:
                        pre_sig = sdf.loc[:sd].tail(vol_check_days + 1)
                        spike = False
                        for idx2 in pre_sig.index[:-1]:  # exclude signal day itself
                            v = float(pre_sig.loc[idx2, 'volume'])
                            vm = float(pre_sig.loc[idx2, 'vol_ma20'])
                            if vm > 0 and v > vm * vol_spike_thresh:
                                spike = True
                                break
                        if spike:
                            self.filter_stats.setdefault('vol_spike_fail', 0)
                            self.filter_stats['vol_spike_fail'] += 1
                            self.filter_stats['filtered_signals'] += 1
                            continue

                    # 波段缩量检查: 信号前5日均量/20日均量 < 阈值(缩量回调)
                    vol_contraction = self.b2_params.get('vol_contraction_ratio', 999.0)
                    if 'volume' in sdf.columns and vol_contraction < 1.0:
                        pre_5d_vol = sdf.loc[:sd].tail(5)['volume'].mean()
                        pre_20d_vol = sdf.loc[:sd].tail(20)['volume'].mean()
                        if pre_20d_vol > 0 and pre_5d_vol / pre_20d_vol > vol_contraction:
                            self.filter_stats.setdefault('vol_contraction_fail', 0)
                            self.filter_stats['vol_contraction_fail'] += 1
                            self.filter_stats['filtered_signals'] += 1
                            continue
    ...
    def calc_position_size(self, signal_weight, buy_price):
        ...
        # Signal ranking: weight-tier allocation
        t1_thresh = self.b2_params.get('t1_threshold', 3.5)
        if signal_weight >= t1_thresh:
            wf *= self.b2_params.get('t1_multiplier', 1.5)
    ...
    def rotate_if_needed(self, signal):
        ...
        should_rotate = False
        rotate_base = self.b2_params.get('rotate_base', 0.3)

        # 1. New signal significantly stronger → rotate
        if signal['weight'] > weakest.signal_weight + rotate_base:
            should_rotate = True
    ...
    def run(self, use_cache=True):
        ...
        if signals is None:
            ...
            if use_cache and len(signals) > 10:
                try:
                    import pickle
                    pickle.dump(signals, open(cache_path, 'wb'))
                except Exception:
                    pass
        ...
        for date in all_trading_dates:
            ...
            # Compute FORTE factor for today
            pass  # FORTE reserved for Enhanced version
            entries_today = 0
            ...
            for s in pending:
                if s['code'] in self.positions:
                    continue
                # 动态有效期: 当日收盘价跌破信号日收盘价阈值 → 自动作废
                df = self.stock_data.get(s['code'])
                expired = False
                if df is not None:
                    if 'date' in df.columns:
                        df = df.set_index('date')
                    if date in df.index:
                        cur_close = float(df.loc[date, 'close'])
                        expire_thresh = self.b2_params.get('sig_expire_thresh', 0.98)
                        if cur_close < s['signal_close'] * expire_thresh:
                            expired = True
                            reason_str = 'sig_expired(c:%.2f<sig:%.2f)' % (cur_close, s['signal_close'])
                            self.trade_log.append({
                                'action': 'SELL', 'date': date, 'code': s['code'],
                                'price': cur_close, 'shares': 0,
                                'proceeds': 0, 'pnl': 0, 'pnl_pct': 0,
                                'reason': reason_str, 'cash_after': self.cash,
                            })
                if expired:
                    continue
                # 硬上限: 超过max_signal_days天仍未买入则强制过期
                max_sig_days = self.b2_params.get('max_signal_days', 7)
                if (date - s['signal_date']).days > max_sig_days:
                    reason_str = 'sig_timeout(T+%d)' % (date - s['signal_date']).days
                    self.trade_log.append({
                        'action': 'SELL', 'date': date, 'code': s['code'],
                        'price': 0, 'shares': 0,
                        'proceeds': 0, 'pnl': 0, 'pnl_pct': 0,
                ...
                if entries_today >= self.max_entries_per_day:
                    ...
                    continue
                if regime == 'defensive' and self.oamv_params.get('oamv_defensive_ban_entry', False):
                    still_pending.append(s)
                    continue
